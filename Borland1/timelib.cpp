/*$T TIMELIB.CPP GC 1.123 09/25/01 14:17:59 */
...
//
// OWLCVT 08/22/97 02:12:39 ObjectWindows - (C) Copyright 1992 by Borland
// International
//
...
void DisplayTime( void ) {
    ...
    CurTime +=
        (
            (s100 - ChessTime[RunColor].sec100) *
            0.01 +
            (sec1 - ChessTime[RunColor].second) +
            (min1 - ChessTime[RunColor].minute) *
            60.0 +
            (hour1 - ChessTime[RunColor].hour) *
            3600.0
        );
...
void StopTime( CLOCKTYPE *clock ) {
    ...
    clock->totaltime +=
        (
            (
                (s100 - clock->sec100) * 0.01 + (sec1 - clock->second) + (min1 - clock->minute) * 60.0 +
