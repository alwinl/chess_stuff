/*$T MOVGEN.CPP GC 1.123 09/25/01 14:17:16 */
...
//
// OWLCVT 08/22/97 02:11:48 ObjectWindows - (C) Copyright 1992 by Borland
// International
//
...
/*
 ...
 =======================================================================================================================
 */
short PieceAttacks( PIECETYPE apiece, COLORTYPE acolor, SQUARETYPE asquare, SQUARETYPE square ) {
    EDGESQUARETYPE  sq;
    int             x;
...
inline short RepeatMove( MOVETYPE *move ) {
    return( move->movpiece != empty && move->movpiece != pawn && move->content == empty && !move->spe );
}

/*
 =======================================================================================================================
    Count the number of moves since last capture or pawn move The game is a draw when fiftymovecnt = 100
 =======================================================================================================================
 */
...
/*
 =======================================================================================================================
    Calculate how many times the position has occured before The game is a draw when repetition = 3;
    movetab[back..Depth] contains the previous moves When immediate is set, only immediate repetion is checked
 =======================================================================================================================
 */
REPEATTYPE Repetition( short immediate ) {
    ...
    do {
        ...
        /*
         * All moves between SAMEDEP and compdep have been checked, so a repetition is
         * found
         */
TEN:
        if( checkdep <= compdep ) {
...
/*
 =======================================================================================================================
    Test whether a move is possible On entry: Move contains a full description of a move, which has been legally
    generated in a different position. MovTab[Depth - 1] contains last performed move. On exit: KillMovGen indicates
        ...
        if( cast & castdir ) {          /* Has king or rook moved before */
            ...
            if( Board[move->new1].piece == empty ) {
                if( Board[castsq].piece == empty ) {
                    if( (move->new1 > move->old) || (Board[move->new1 - 1].piece == empty) ) {  /* Are the squares
                                                                                                 * unattacked */
                    ...
                    if( Board[move->new1].piece == move->content ) {
                        if( move->content == empty || Board[move->new1].color == Opponent ) {
                            if( move->movpiece == pawn ) {  /* Is the move possible? */
                                ...
                                killmov = PieceAttacks( move->movpiece, Player, move->old, move->new1 );
                        }
                    }
                }
            }
...
/*
 =======================================================================================================================
    The move generator. InitMovGen generates all possible moves and places them in a buffer. Movgen will the generate
    the moves one by one and place them in next. On entry: Player contains the color to move. MovTab[Depth-1] the las
    performed move. On exit: Buffer contains the generated moves. The moves are generated in the order:: Captures
    Castlings Non captures E.p. captures
 =======================================================================================================================
 */
...
/*
 =======================================================================================================================
    place next move from the buffer in next. Generate zeromove when there are no more moves.
 =======================================================================================================================
 */
void MovGen( void ) {
    if( BufPnt >= BufCount )
