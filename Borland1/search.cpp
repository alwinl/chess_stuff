/*$T SEARCH.CPP GC 1.123 09/25/01 14:17:34 */
...
//
// OWLCVT 08/22/97 02:12:18 ObjectWindows - (C) Copyright 1992 by Borland
// International
//
...
static void MessageScan( void ) {
    ...
    if( Analysis ) {
        switch( msg.message )
        {
        case WM_SETCURSOR:  DispatchMessage( &msg ); break;

        case WM_COMMAND:    switch( msg.wParam ){ case CM_STOP:   SkipSearch = TRUE; AutoPlay = FALSE; break; }break;
...
static MAXTYPE  search( MAXTYPE alpha, MAXTYPE beta, int ply, INFTYPE *inf, MOVETYPE *bestline );
...
static short epcapmovgen( PARAMTYPE *P )
{
    ...
    if( MovTab[Depth - 1].movpiece == pawn ) {
        if( abs(MovTab[Depth - 1].new1 - MovTab[Depth - 1].old) >= 0x20 ) {
            ...
            MovTab[Depth].content = empty;
            MovTab[Depth].new1 = ( MovTab[Depth - 1].new1 + MovTab[Depth - 1].old ) / 2;    // Square that the pawn
                                                                                            ///hopped over
    ...
    /*
     ===================================================================================================================
        Generate the next move to be analysed. Controls the order of the movegeneration. The moves are generated in the
        order: Main variation Captures of last moved piece Killing moves Other captures Pawnpromotions Castling Normal
        moves E.p. captures
     ===================================================================================================================
     */
    static void searchmovgen( PARAMTYPE *P ) {
        ...
        (
            P->S->capturesearch
        &&  (passedpawn[Depth - 2] >= 0)
        &&  (Board[passedpawn[Depth - 2]].piece == pawn && Board[passedpawn[Depth - 2]].color == Player)
        ) {
    ...
    /*
     ===================================================================================================================
        Perform the search On entry:: Player is next to move MovTab[Depth-1] contains last move alpha, beta contains the
        alpha - beta window ply contains the Depth of the search inf contains various information On exit:: Bestline
        contains the principal variation search contains the evaluation for Player
     ===================================================================================================================
     */
    static MAXTYPE search( MAXTYPE alpha, MAXTYPE beta, int ply, INFTYPE *inf, MOVETYPE *bestline ) {
        SEARCHTYPE  S;
