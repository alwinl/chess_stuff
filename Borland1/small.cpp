/*$T SMALL.CPP GC 1.123 09/25/01 14:17:43 */
...
//
// OWLCVT 08/22/97 02:12:25 ObjectWindows - (C) Copyright 1992 by Borland
// International
//
...
/*
 =======================================================================================================================
    Test if the move is legal for Programcolor == player in the given position
 =======================================================================================================================
 */
...
void PrintComment( void ) {
    ...
    if( check && !checkmate )
        ...
        if( -25500 < HintEvalu && HintEvalu < -0x880 ) {
            switch( Opponent )
            {
            case white: EndMessage( " White resigns" ); break;
            case black: EndMessage( " Black resigns" );
            }
        }
    }
}
...
void StartAnalysis( void ) {
    ...
    case normal:
        /*
         * Divides the Time left till nest time control between moves left. There is a
         * margin of 4 moves to make sure that the program does not lose on time
         */
        timecontrol = ( ((MoveNo >> 1) + 20) / 20 ) * 20;
        if( timecontrol <= 40 )
            timecontrol = 40;
        WantedTime = ( AverageTime * timecontrol - ChessTime[ProgramColor].totaltime ) / ( timecontrol + 4 - (MoveNo >> 1) );

        /*
         * In the begining of the game the program thinks around twice as long, since the
         * early middle game is normally the most crucial part of a game
         */
        if( (MoveNo >> 1) <= 40 )
    ...
    case fullgametime:
        /*
         * Assumes that the game will last for around 40 moves and divides the time left
         * accordingly
         */
    ...
    case matching:
        ...
        if( MoveNo >= 2 )
            ...
            WantedTime = 5.0;
        WantedTime += ( ChessTime[Opponent].totaltime - ChessTime[ProgramColor].totaltime ) * 0.25;
    default:
