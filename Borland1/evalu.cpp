...
static int pawnstrval( short depth, ENUMCOLOR color )
{
    unsigned char iso;  /*  contains FILEs with isolated pawns  */

    iso = pawnbit[ depth ][ color ].one & ( ~( (pawnbit[ depth ][ color ].one << 1) | (pawnbit[ depth ][ color ].one >> 1) ) );

    return( -( count( pawnbit[ depth ][ color ].dob) * DOUBLEPAWN + count( iso ) * ISOLATEDPAWN + count( iso & pawnbit[ depth ][ color ].dob ) * ISOLATEDPAWN * 2 ) );
}
...
void CalcPVTable( void )
{
    ...
    material = pawntotalmaterial = totalmaterial = 0;

    for( square = 0; square < 0x78; square++ ) {
        if( !( square & 0x88 ) && Board[ square ].piece != empty && Board[ square ].piece != king ) {
