/*$T DRAG.CPP GC 1.123 09/25/01 14:16:54 */
...
//
// OWLCVT 08/22/97 02:11:22 ObjectWindows - (C) Copyright 1992 by Borland
// International
//
...
static void SaveBackground( POINT &p ) {
    ...
    hOldBmp = ( HBITMAP ) SelectObject( hSaveDC, CreateCompatibleBitmap(hDC, Bitmap.bmWidth, Bitmap.bmHeight) );
...
static void RestoreBackground( void ) {
    ...
    BitBlt( hDC, LastPoint.x, LastPoint.y, Bitmap.bmWidth, Bitmap.bmHeight, hSaveDC, 0, 0, SRCCOPY );
...
static void ShowPiece( POINT &p ) {
    ...
    BitBlt( hDC, p.x, p.y, Bitmap.bmWidth, Bitmap.bmHeight, hMemoryDC, 0, 0, SRCINVERT );
...
static void UpdateScreen( POINT &p ) {
    ...
    hNewOldBmp = ( HBITMAP ) SelectObject( hNewSaveDC, CreateCompatibleBitmap(hDC, Bitmap.bmWidth, Bitmap.bmHeight) );
    ...
    /* restore old section */
    BitBlt( hUpdateDC, abs(rect.left - rect2.left), abs(rect.top - rect2.top), Bitmap.bmWidth, Bitmap.bmHeight, hSaveDC,
           0, 0, SRCCOPY );

    /* save new background */
    BitBlt( hNewSaveDC, 0, 0, Bitmap.bmWidth, Bitmap.bmHeight, hUpdateDC, abs(rect.left - rect1.left),
           abs(rect.top - rect1.top), SRCCOPY );

    /* blit the mask */
    DeleteObject( SelectObject(hSaveDC, hDragMask) );

    BitBlt( hUpdateDC, abs(rect.left - rect1.left), abs(rect.top - rect1.top), Bitmap.bmWidth, Bitmap.bmHeight, hSaveDC,
           0, 0, SRCAND );
