...
short Check(SQUARETYPE square, PIECETYPE inpiece, COLORTYPE incolor)
{
    ...
    {
        dep = Depth - 1;
        while (MovTab[dep].movpiece != no_piece)
        {
...
{
    return (move->movpiece != no_piece && move->movpiece != pawn &&
              move->content == no_piece && !move->spe);
}
...
short KillMovGen(MOVETYPE *move)
{
    ...
    if (move->spe && (move->movpiece == king))
    {
        ...
        if (cast & castdir)    /*  Has king or rook moved before  */
        {
            ...
            if  (Board[move->new1].piece == no_piece)
        ...
        if (Board[move->old].piece == move->movpiece)
          if (Board[move->old].color == Player)
            if (Board[move->new1].piece == move->content)
              if (move->content == no_piece || Board[move->new1].color == Opponent)
              {
                if (move->movpiece == pawn)   /*  Is the move possible?  */
                {
                  if (abs(move->new1 - move->old) < 0x20)
                    ...
                    killmov = Board[(move->new1+move->old) / 2].piece == no_piece;
                }
                else
...
static void CapMovGen(void)
{
    ...
    for (i = OfficerNo[Player]; i >= 0; i--)
        if (PieceTab[Player][i].ipiece != no_piece && PieceTab[Player][i].ipiece
                != pawn)
    ...
    switch (Next.movpiece)
    {
        ...
        case bishop:
            ...
            for (dir = first; dir >= last; dir--)
            {
                ...
                while (!(newsq & 0x88))
                {
                    if (Board[newsq].piece != no_piece) goto TEN;
                    Next.new1 = newsq;
                    Generate();
            ...
            if (Board[Next.new1].piece == no_piece)
            {
                ...
                else
                {
                    ...
                    if (Next.old < 0x18 || Next.old >= 0x60)
                    {
                        ...
                        if (Board[Next.new1].piece == no_piece) Generate();
                    }
                }
            }
    }  /* switch */
