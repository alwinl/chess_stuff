...
TNoEraseBkGndStatic::TNoEraseBkGndStatic( PTWindows AParent, int AnId, LPSTR ATitle, int X, int Y, int W, int H, WORD ATextLen, PTModule AModule = NULL )
...
void TInfoWindow::DrawSFrame( HDC hDC, RECT *rect )
{
    ...
    pArray[0].x = x1; pArray[0].y = y2; // left bottom
    pArray[1].x = x1; pArray[1].y = y1; // left top
    pArray[2].x = x2; pArray[2].y = y1; // right top

    Polyline( hDC, pArray, 3 );

    DeleteObject( SelectObject( hDC, GetStockObject(WHITE_PEN)) );

    pArray[1].x = x2; pArray[1].y = y2; //right bottom
    pArray[2].y = y1 - 1;
...
void TInfoWindow::DrawStaticFrames( HDC hDC )
{
   ...
   rect.left = LEFTMARGIN - 1; rect.right = MAXLINE + 1;

   rect.top = COLORY - 1;     rect.bottom = COLORY + LINESIZE + 1;     DrawSFrame( hDC, &rect );
   rect.top = WHITEINFOY - 1; rect.bottom = WHITEINFOY + LINESIZE + 1; DrawSFrame( hDC, &rect );
   rect.top = BLACKINFOY - 1; rect.bottom = BLACKINFOY + LINESIZE + 1; DrawSFrame( hDC, &rect );
   rect.top = TIMERY - 1;     rect.bottom = TIMERY + LINESIZE + 1;     DrawSFrame( hDC, &rect );
   rect.top = LEVELY - 1;     rect.bottom = LEVELY + LINESIZE + 1;     DrawSFrame( hDC, &rect );
   rect.top = VALUEY - 1;     rect.bottom = VALUEY + LINESIZE + 1;     DrawSFrame( hDC, &rect );
   rect.top = NODEY - 1;      rect.bottom = NODEY + LINESIZE + 1;      DrawSFrame( hDC, &rect );
   rect.top = SECY - 1;       rect.bottom = SECY + LINESIZE + 1;       DrawSFrame( hDC, &rect );
   rect.top = DEPTHY - 1;     rect.bottom = DEPTHY + LINESIZE + 1;     DrawSFrame( hDC, &rect );
   rect.top = BESTY - 1;      rect.bottom = BESTY + BESTH + 1;         DrawSFrame( hDC, &rect );
   rect.top = MESSY - 1;      rect.bottom = MESSY + LINESIZE + 1;      DrawSFrame( hDC, &rect );
}
