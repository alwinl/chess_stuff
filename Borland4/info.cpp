...
BOOL TNoEraseBkGndStatic::EvEraseBkgnd( HDC )
{
...
TInfoWindow::TInfoWindow(TWindow * AParent, LPSTR ATitle) : TFrameWindow(AParent, ATitle)
{
	short temp;
	...
	txtDepth  = new TNoEraseBkGndStatic( this, -1, EmptyStr, DEPTHX, DEPTHY, DEPTHW, LINESIZE, DEPTHW /CHARSIZE);
...
void TInfoWindow::Paint(HDC PaintDC, PAINTSTRUCT _FAR & )
{
...
HBRUSH TInfoWindow::EvCtlColor( HDC, HWND, UINT )
{
	TMessage Msg = __GetTMessage();
...
void TInfoWindow::DrawSFrame(HDC hDC, RECT *rect)
{
	...
	pArray[0].x = rect->left;	pArray[0].y = rect->bottom;
	pArray[1].x = rect->left;	pArray[1].y = rect->top;
	pArray[2].x = rect->right;	pArray[2].y = rect->top;

	Polyline(hDC, pArray, 3);

	DeleteObject( SelectObject( hDC, GetStockObject( WHITE_PEN ) ) );

	pArray[1].x = rect->right; 	pArray[1].y = rect->bottom;
