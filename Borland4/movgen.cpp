...
short PieceAttacks(ENUMPIECE apiece, ENUMCOLOR acolor,
                     int asquare, int square)
{
    int sq;
    int x;
...
short PawnAttacks( ENUMCOLOR acolor, int square )
{
    int sq;

    sq = square - ( (acolor == white) ? 0x10 : -0x10 ) - 1;  /*  left square  */
    if( VALIDSQUARE( sq ) )
...
short Check(int square, ENUMPIECE inpiece, ENUMCOLOR incolor)
{
    short dep;
...
inline short RepeatMove(MOVESTRUCT *move)
{
...
unsigned char FiftyMoveCnt(void)
{
    unsigned char cnt = 0;
...
unsigned char Repetition(short immediate)
{
...
short KillMovGen(MOVESTRUCT *move)
{
    int castsq;
    ENUMPIECE promote;
    ENUMCASTDIR castdir;
...
TEN:            continue;
            ...
            break;
        case pawn :
            Next.new1 = Next.old + ( (Player == white) ? 0x10 : -0x10 );  /*  one square forward  */
