...
MOVESTRUCT ZeroMove = { 8, 8, 0, empty, empty };
...
BOOL IllegalMove(MOVESTRUCT *move)
{
   BOOL illegal;
...
void EnterMove(MOVESTRUCT *move)
{
   StopChessTime();
...
void RemoveMove(MOVESTRUCT *move)
{
   StopChessTime();
   PrintMove(MoveNo, ProgramColor, move, ChessTime[RunColor].totaltime);
   TakeBackMove(move);
...
void TakeBackMove(MOVESTRUCT *move)
{
    ProgramColor = Opponent;
    Opponent = Player;
    Player = ProgramColor;
...
void FlashMove(MOVESTRUCT *move)
{
    MakeMove(move);
    UpdateBoard();
    Wait(4);
...
void DoSlideMove(MOVESTRUCT &move)
{
