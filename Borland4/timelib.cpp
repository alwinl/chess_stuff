...
void DisplayTime()
{
   ...
   CurTime += ((s100 - ChessTime[RunColor].sec100) * 0.01 + (sec1 - ChessTime[RunColor].second) 
				+ (min1 - ChessTime[RunColor].minute) * 60.0 + (hour1 - ChessTime[RunColor].hour) * 3600.0);
...
void StopTime(CLOCKTYPE *clock)
{
    ...
    clock->totaltime  += ( ((s100 - clock->sec100) * 0.01 + (sec1 - clock->second) 
						+ (min1 - clock->minute) * 60.0 + (hour1 - clock->hour) * 3600.0) );
    clock->hour        = hour1;
    clock->minute      = min1;
    clock->second      = sec1;
    clock->sec100      = s100;
