...
void PrintMove(int moveno, ENUMCOLOR programcolor, MOVESTRUCT *move, double time)
{
   int minutes;
...
void DrawBoard()
{
   unsigned char no;
   HDC hDC;
   const int printno[64] = { 0, 0x10, 0x20, 0x30, 0x40, 0x50,
...
void PrintPiece(int square, ENUMPIECE piece, ENUMCOLOR color, DWORD Rop)
{
...
int GetValidSquare(POINT p, ENUMCOLOR player, BOOL CheckPiece)
{
   POINT point;
   int square;
   RECT sqrect;
...
void PrintBestMove(MOVESTRUCT *mainline, int mainevalu)
{
   short dep = 0;
